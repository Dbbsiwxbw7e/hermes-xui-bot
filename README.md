...
| بخش | کارها |
|---|---|
| 👤 **اکانت‌ها** | افزودن چند اکانت Railway، سوییچ، حذف — دائمی ذخیره میشه |
| 🚀 **دپلوی** | ساخت پنل‌ها، اتصال خودکار نودها، ست دامنه، حذف |
| 🛰 **TCP Proxy** | چرخش با تعداد/پورت دلخواه + لیست دامنه‌های قابل ویرایش |
| 📥 **اینباندها** | ساخت/لیست/حذف اینباند VLESS+TLS + لینک آماده |

دستورات سریع: `/tcp` `/link` `/status` `/delete` — بقیه از منو.
